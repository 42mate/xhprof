...
if ($domain_weight = getenv('xhprof_weight')) { //Overrides weight config from apache confs.
...
function print_flat_data($url_params, $title, $flat_data, $sort, $run1, $run2, $limit) {
  ...
  include("../lib/templates/profChart.phtml");
  include("../lib/templates/profTable.phtml");
...
function full_report($url_params, $symbol_tab, $sort, $run1, $run2, $links) {
  ...
  if ($diff_mode) {
    ...
    include "../lib/templates/diff_run_header_block.phtml";

  }
  else {
    global $xhprof_runs_impl;
    include "../lib/templates/single_run_header_block.phtml";
  }
