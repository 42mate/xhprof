...
if ($controlIPs === FALSE || in_array($_SERVER['REMOTE_ADDR'], $controlIPs) || PHP_SAPI == 'cli') {
  ...
  if (isset($_COOKIE['_profile']) && $_COOKIE['_profile'] || PHP_SAPI == 'cli'
    && ((isset($_SERVER['XHPROF_PROFILE'])
    && $_SERVER['XHPROF_PROFILE']) || (isset($_ENV['XHPROF_PROFILE']) && $_ENV['XHPROF_PROFILE']))) {
...
//Determine whether or not to profile this URL randomly
...
if (extension_loaded('xhprof') && $_xhprof['doprofile'] === TRUE) {
  include_once dirname(__FILE__) . '/../lib/utils/xhprof_lib.php';
  include_once dirname(__FILE__) . '/../lib/utils/xhprof_runs.php';
